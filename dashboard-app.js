                ...
                <div className="min-h-screen flex items-center justify-center bg-gray-50">
                    <div className="text-center p-8 bg-white shadow-lg rounded-xl">
                        <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                            <i data-lucide="alert-triangle" className="text-red-600 w-8 h-8"></i>
                        </div>
                        <h1 className="text-2xl font-bold text-gray-900 mb-2">Something went wrong</h1>
                        <p className="text-gray-600 mb-6 max-w-sm mx-auto">
                            The application encountered an unexpected error.
                        </p>
                        <p className="text-red-500 text-xs mb-6 bg-red-50 p-2 rounded border border-red-100 font-mono text-left overflow-auto max-h-32">
...
function Dashboard() {
    ...
    React.useEffect(() => {
        ...
        Promise.all([
            ...
            if (announce.length > 0) {
                ...
                if (latest.date === today && user && user.role === 'faculty') {
                    // Simple simulated "Push" notification
                    if (!localStorage.getItem(`seen_announcement_${latest.id}`)) {
    ...
    const handleDismissNotification = () => {
        if (showNotification) {
            localStorage.setItem(`seen_announcement_${showNotification.id}`, 'true');
    ...
    // Trigger Icon Creation - Removed as we use LucideIcon now

    // Safety check for user before rendering
    if (!user) {
        return <div className="flex h-screen items-center justify-center text-gray-500">Redirecting to login...</div>;
    }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="card bg-gradient-to-br from-purple-500 to-indigo-600 text-white border-none">
                            <div className="flex justify-between items-start">
                                ...
                                <LucideIcon name="building-2" className="text-3xl opacity-80" />
                            </div>
                        </div>
                        <div className="card bg-gradient-to-br from-emerald-500 to-teal-600 text-white border-none">
                            <div className="flex justify-between items-start">
                                ...
                                <LucideIcon name="user-check" className="text-3xl opacity-80" />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    {[
                        ...
                        <div key={stat.label} className="card flex items-center p-6 hover:shadow-md transition-shadow">
                            <div className={`w-14 h-14 rounded-full ${stat.color} bg-opacity-10 flex items-center justify-center mr-4`}>
                                <LucideIcon name={stat.icon} className={`text-2xl ${stat.color.replace('bg-', 'text-')}`} />
                            </div>
                            <div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="card bg-blue-50 border-blue-100">
                            <h3 className="text-lg font-bold text-blue-900 mb-4 flex items-center">
                                <LucideIcon name="megaphone" className="mr-2 w-5 h-5" />
                                Post Announcement
                            </h3>
                        ...
                        <div className="card h-fit">
                            ...
                            {requests.length > 0 ? (
                                    ...
                                    {requests.map(req => (
                                        <div key={req.id} className="bg-amber-50 border border-amber-100 p-3 rounded-lg text-sm">
                                            <div className="flex justify-between items-start mb-1">
                                                ...
                                                <span className="text-xs text-gray-400">{req.createdAt.split(' ')[0]}</span>
                                            </div>
                                ...
                                <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                    <LucideIcon name="inbox" className="text-4xl text-gray-300 mx-auto mb-2 w-10 h-10" />
